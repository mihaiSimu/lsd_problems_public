...
(* We consider a graph as a map between an integer (a node) and a set of integers (the nodes it has edges towards) 
  ...
  grah_contains_cycle mp 
  returns true/false expressing weather there is a cycle in the graph 
  (succession of edes starting at a node and ending at the same node)
*)

let grah_contains_cycle mp = failwith "Not Implemented";;

(* ========================= Tests: =========================== *)
let test_graph_no_cycles = create_int_map [
  (1, (IntS.of_list [2]));
...
let test_graph_has_cycles = create_int_map [
  (1, (IntS.of_list [2]));
  (2, (IntS.of_list [3;4;7]));
  (3, (IntS.of_list [5]));
  (4, (IntS.of_list []));
...
testExpected (grah_contains_cycle test_graph_no_cycles) false "Test that a graph contains no cycle" ;;
testExpected (grah_contains_cycle test_graph_has_cycles) true "Test that a graph contains at least one cycle" ;;
