...
(* We consider a graph as a map between an integer (a node) and a set of integers (the nodes it has edges towards) 
  ...
  dfs mp start
  Returns a list representing the DFS-traversal of the graph

  suggestion: create a dfs_aux function that takes as parameter the current node and an object representing the list-of-dfs-traversal

*)

let dfs mp start = failwith "Not Implemented";;
