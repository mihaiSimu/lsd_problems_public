...
(* We consider a graph as a map between an integer (a node) and a set of integers (the nodes it has edges towards) 
  ...
  exists_road mp a b 
  Returns true or false depending on weather there is a road (succession of edges) leading from a to b
*)

let exists_road mp a b = failwith "Not Implemented";;
...
testExpected (exists_road test_graph 1 6) true "There is a road from 1 to 6" ;;
testExpected (exists_road test_graph 5 8) true "There is a road from 5 to 8" ;;
testExpected (exists_road test_graph 6 1) false "No road from 5 to 8" ;;
testExpected (exists_road test_graph 6 6) false "No road from 6 to 6" ;;